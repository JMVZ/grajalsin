    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-green-600 mt-1">Modifica la información del producto</p>
            </div>
    ...
    <div class="space-y-6">
        <div class="bg-white shadow rounded-lg border border-green-200">
            <div class="px-4 py-5 sm:p-6">
                <form action="{{ route('inventario.productos.update', $producto) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="descripcion" id="descripcion" rows="3"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500">{{ old('descripcion', $producto->descripcion) }}</textarea>
                            @error('descripcion')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <select name="unidad_medida" id="unidad_medida" required
                                ...
                                <option value="quintal" {{ old('unidad_medida', $producto->unidad_medida) == 'quintal' ? 'selected' : '' }}>Quintales</option>
                                <option value="costal" {{ old('unidad_medida', $producto->unidad_medida) == 'costal' ? 'selected' : '' }}>Costales</option>
                                <option value="tonelada" {{ old('unidad_medida', $producto->unidad_medida) == 'tonelada' ? 'selected' : '' }}>Toneladas</option>
                                <option value="libra" {{ old('unidad_medida', $producto->unidad_medida) == 'libra' ? 'selected' : '' }}>Libras</option>
                            </select>
                            @error('unidad_medida')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div id="stock-fields" class="md:col-span-2 grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div>
                                ...
                                <input type="number" name="stock_actual" id="stock_actual" step="0.01" value="{{ old('stock_actual', $producto->stock_actual) }}"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500">
                            </div>

                            <div>
                                <label for="stock_minimo" class="block text-sm font-medium text-gray-700">Stock Mínimo (Alerta)</label>
                                <input type="number" name="stock_minimo" id="stock_minimo" step="0.01" value="{{ old('stock_minimo', $producto->stock_minimo) }}"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500">
                            </div>

                            <div>
                                <label for="stock_maximo" class="block text-sm font-medium text-gray-700">Stock Máximo</label>
                                <input type="number" name="stock_maximo" id="stock_maximo" step="0.01" value="{{ old('stock_maximo', $producto->stock_maximo) }}"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500">
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="mt-1 relative rounded-md shadow-sm">
                                ...
                                <input type="number" name="precio_compra" id="precio_compra" step="0.01" value="{{ old('precio_compra', $producto->precio_compra) }}"
                        ...
                        <div>
                            ...
                            <div class="mt-1 relative rounded-md shadow-sm">
                                ...
                                <input type="number" name="precio_venta" id="precio_venta" step="0.01" value="{{ old('precio_venta', $producto->precio_venta) }}"
                                    class="pl-7 block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500">
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="ubicacion" id="ubicacion"
                                ...
                                @foreach(\App\Models\Bodega::where('estatus', true)->orderBy('nombre')->get() as $bodega)
                                    <option value="{{ $bodega->nombre }}" {{ old('ubicacion', $producto->ubicacion) == $bodega->nombre ? 'selected' : '' }}>
                        ...
                        <div>
                            <div class="flex items-start">
                                ...
                                <div class="ml-3 text-sm mt-6">
                                    <label for="activo" class="font-medium text-gray-700">Producto Activo</label>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="md:col-span-2">
                            ...
                            @if($producto->imagen)
                                <div class="mt-2 mb-2">
                                    <img src="{{ Storage::url($producto->imagen) }}" alt="{{ $producto->nombre }}" class="h-20 w-20 object-cover rounded">
                                </div>
                            @endif
                            ...
                            <p class="text-xs text-gray-500 mt-1">Deja en blanco si no deseas cambiar la imagen</p>
                        </div>
                    </div>
