<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Registrar pago — {{ $cliente->nombre }}</h2>
            <a href="{{ route('cobranza.show', $cliente) }}" class="px-4 py-2 rounded-md border border-gray-300 text-gray-700 hover:bg-gray-50 text-sm">← Estado de cuenta</a>
        </div>
    </x-slot>

    <div class="py-6">
        <div class="max-w-xl mx-auto">
            <p class="text-sm text-gray-600 mb-4">Saldo actual: <strong class="text-red-600">${{ number_format($saldo, 2) }}</strong></p>
            <div class="bg-white shadow-sm rounded-lg p-6">
                <form method="POST" action="{{ route('cobranza.pago.store', $cliente) }}">
                    @csrf
                    <div class="space-y-4">
                        <div>
                    ...
                    <div class="mt-6 flex justify-end space-x-2">
                        <a href="{{ route('cobranza.show', $cliente) }}" class="px-4 py-2 rounded-md border border-gray-300 text-gray-700 hover:bg-gray-50">Cancelar</a>
