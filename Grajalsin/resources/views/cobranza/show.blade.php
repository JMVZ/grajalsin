<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <div>
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">Estado de cuenta — {{ $cliente->nombre }}</h2>
                <p class="text-sm text-gray-500 mt-1">Código: {{ $cliente->codigo ?? '—' }}</p>
            </div>
            <div class="flex space-x-2">
                <a href="{{ route('cobranza.index') }}" class="px-4 py-2 rounded-md border border-gray-300 text-gray-700 hover:bg-gray-50 text-sm">← Cobranza</a>
                @if($saldo > 0)
                    <a href="{{ route('cobranza.pago.create', $cliente) }}" class="px-4 py-2 bg-green-600 text-white rounded-md text-sm hover:bg-green-700">Registrar pago</a>
                @endif
            </div>
        </div>
    </x-slot>
    ...
    <div class="py-6">
        <div class="max-w-4xl mx-auto">
            ...
            <div class="bg-white shadow-sm rounded-lg p-6 mb-6">
                <p class="text-sm text-gray-500">Saldo (lo que el cliente debe a Grajalsin)</p>
            ...
            <div class="bg-white shadow-sm rounded-lg overflow-hidden">
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($cliente->movimientosCuenta as $mov)
                            <tr>
